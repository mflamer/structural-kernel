...
**Status:** Approved 2026-07-07 with required revisions R1–R3 (`0001-review-response.md`);
revisions applied 2026-07-07 per that review and the product-owner reply
(`0001-review-findings-reply.md`). All §10 open questions are answered in the review.
Derivation implementation remains gated on the eid identity-scheme ADR (R1); proposal
at `0002-eid-identity-scheme.md`.
...
- `exception` — a targeted rule override of another decision's output at a specific
  ...
  from reality overrides (§5), which record *measurement*, not preference. An exception
  references its target by `eid`. If a re-derivation makes that eid cease to exist, the
  exception is **dangling** — and the semantics are deliberately asymmetric to override
  dangling (§5): a dangling override is stale *measurement* (warning, persists inert),
  while a dangling exception is deliberate *design will* whose target vanished. It
  surfaces as a distinct validation error (`dangling_exception`), and the changeset
  that orphans it must explicitly resolve it — retarget or delete — to commit. Quiet
  inertness is not an option here; the mechanisms are separate because the semantics
  genuinely differ (per review R3 and the PO reply).
...
- **Total over valid snapshots**: any snapshot that passed commit-time validation must
  ...
  Totality includes **partial derived models**: a snapshot containing an explicitly
  *unresolved* decision ("structural system: open") derives to everything derivable,
  with the openness explicitly represented in the output — never an error. A partial
  model may legitimately yield no solvable analysis artifact (absence is a valid state,
  not a failure), and queries operate on partial models (standing requirement 10).
- **Staged, in contract shape**: phase 1 implements a single snapshot → model pass, but
  the contract is a sequence of pure stages in which later stages may take solve results
  as inputs (derive → solve → detail-derive → re-solve where detailing changes
  stiffness). The type signature must not foreclose this (standing requirement 7).
  Likewise, level of detail is a derivation *parameter* — part of the cache key, so the
  same snapshot derives at exploration or fabrication resolution as distinct cached
  artifacts (standing requirement 8).
...
to *stable anchors* rather than array position, so local edits perturb local `eid`s
only. Overrides and intents reference `eid`s; this scheme is what keeps a surveyed
override pinned to *that joist* across re-derivations.

The exact anchor scheme gets its own ADR **before any derivation implementation**
(review R1). That ADR must resolve the gridline-move scenario — moving gridline B 2 ft
east must not destroy cross-branch eid correspondence, or "J5 got heavier in variant 3"
is unanswerable — and may define different correspondence semantics for the two
consumers: overrides pin to absolute/surveyed space (dangling on a grid move is
arguably *correct* — the surveyed member didn't move when the grid abstraction did),
while diffs and exploration rankings correspond in rule-relative space. It must also
anticipate hierarchical component identity — bolt under connection under member
(standing requirement 8) — so granular eids nest rather than requiring a second
identity system. Proposal: `docs/design/0002-eid-identity-scheme.md`.
...
environment, or network reads, no mutable captured state — anything outside that tuple
silently breaks determinism and replayability. The contract is property-tested the same
way derivation determinism is: run twice over identical inputs ⇒ identical violations.

**Two-site enforcement (PO reply, item 1):** intent enforcement is split by what each
site can decide.

- *Commit-time intent checkers* (this section) are restricted to what is decidable from
  the dry-run derived model and snapshot — topology, load-path connectivity,
  referential shape — under the purity tuple above, unchanged.
- *Solve-time design checks* verify demand-dependent limits — deflection (L/360 live,
  L/240 total) and ASD unity — over `SolveResult`s, enforced as hard exploration
  constraints.

Both sites must cite the intent instance they enforce, so a failed deflection check
traces to the `serviceability` intent in the audit trail. That linkage — not a
commit-time displacement check, which is impossible before a solve exists — is the
substance of the requirement that phase 1's `serviceability` category ships real, not
as a placeholder.
...
- **`SolveResult`** (per artifact, per combo): nodal displacements, reactions, member
  ...
  Every `SolveResult` carries an **engine class / fidelity field** from day one
  (phase 1: xara, verification-grade), so screening-grade engines arrive later
  without schema surgery; only verification-grade results feed design checks and
  the engineer-of-record record (standing requirement 9).
...
```jsonc
{
  ...
  "generations": [ {
      ...
      "candidates": [ {
          ...
          "artifact": "sha256:...",
          "result":   "sha256:..."         // physics only — evaluation is a separate layer
      } ]
  } ],
  "evaluations": [ {                        // keyed by (result set, cost_basis)
      "result_set": "sha256:...",           // hash over the solve-result set evaluated
      "cost_basis": "01JCB... | null",      // versioned cost_basis decision; null ⇒ physics-only metrics (phase 1)
      "per_candidate": { "c3": { "total_mass": {...}, "max_unity": 0.87, "feasible": true } },
      "ranking": ["c3", "c1", ...],
      "uncertainty": { /* basis citation + noise statement, standing req. 6 */ }
  } ]
}
```

- **Evaluation is a layer distinct from solving (PO reply, item 3).** Candidates carry
  changeset, branch, rationale, artifact, and solve-result references only — physics.
  Evaluations are a separate collection on the exploration keyed by
  `(result set, cost_basis)`, each with its own ranking. Re-ranking under a revised
  basis *appends* an evaluation; it never re-solves and never mutates candidates.
  Phase 1's objective (minimize material weight) is an evaluation with a null cost
  basis; nothing in the schema assumes candidates share a proposer strategy or decision
  kind (standing requirement 1).
...
*(All answered in `0001-review-response.md`, 2026-07-07: Q1 wood — sawn lumber, NDS
ASD; Q2 ASCE 7-22 §2.4 combos; Q3 yes — L/360 live and L/240 total as hard phase-1
constraints; Q4–Q9 confirmed as proposed; Q10 proceed with xara, email drafted,
non-blocking. Preserved below as originally posed, for the record.)*
